...
export class GithubSha256Middleware {
  ...
  static async verifyGithubSignature(req: Request, res: Response, next: NextFunction) {
    ...
    const secret = APP_CONFIG.WEBHOOK_SECRET ?? "";


    if(await GithubSha256Middleware.verifySignature(secret, xHubSignature, body)){
        return next()
    }   
  ...
  static async verifySignature(secret: string, header: string, payload: string) {
    try {
